#!/usr/bin/env python
## ------------------------------------------------------
## Adapted from CliMT (written by Rodrigo Caballero Augi)
## ------------------------------------------------------
...
def getSources(dir):
    ...
    if os.path.exists(SrcFile):
        ...
        Sources = []
        for pattern in ['*.f','*.F','*.f90','*.F90','*.f95','*.F95']:
...
def buildNeeded(target,src):
    ...
    target = os.path.join('lib/PCE',target)
...
def build_ext(name=None, dir=None, cppflags='', f77flags='', f90flags='', \
    ...
    src = getSources(dir)
    target = '_%s.so' % name
    driver = glob.glob(os.path.join(dir,'hermPol.f90'))[0]
...
#        F2pyCommand.append('/usr/bin/f2py -c -m _%s' % name)
        ...
        if os.system(F2pyCommand) > 0:
            ...
            sys.exit()
        os.system('mv -f _%s.so lib/PCE' % name)
...
setup(name         = "PCE",
      ...
      author       = "Sebatian Gimeno Garcia",
      author_email = "sebastian.gimenogarcia@dlr.de",
      url          = "http://atmos.caf.dlr.de",
      packages    = ['PCE'],
      package_dir = {'':'lib'},
      package_data = {'PCE':['*.so']+DataFiles})
